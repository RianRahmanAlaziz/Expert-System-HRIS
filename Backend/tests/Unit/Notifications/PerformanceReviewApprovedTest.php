...
class PerformanceReviewApprovedTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $notification = new PerformanceReviewApproved(
            $this->createReview()
        );
    ...
    public function test_it_contains_correct_database_data(): void
    {
        ...
        $notification = new PerformanceReviewApproved($review);

        $data = $notification->toDatabase(new User());

        $this->assertSame(
            'Performance Review Disetujui',
            $data['title']
        );

        $this->assertSame(
            'Performance review Anda untuk periode Performance Period 2026 telah disetujui.',
            $data['message']
        );
